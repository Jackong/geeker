...
class Vcode {
    ...
    public function get($id) {
        ...
        if ($result['ret'] == '-303') {
            Log::debug("$uid|$account|$password|waiting vcode result|" . json_encode($result));
            echo "gkWaitVCode('$id')";
            return;
        }
        $code = $result['result'];
    ...
    public function gets() {
        ...
        Log::debug("tieba|vcode|$uid|$account|$url");
        $data = $crawler->crawl($url, $handler, false);
        if (is_null($data)) {
            Log::error("$uid|$account|$password|can not download vcode image|$url");
            echo 'gkVCode()';
            return;
        }
        ...
        if (false === file_put_contents($file, $data)) {
            Log::error("$uid|$account|$password|can not save vcode image|$file|$url");
            echo "gkVCode()";
            return;
        }
        $damaApi = new \Dama2Api($account, $password);
